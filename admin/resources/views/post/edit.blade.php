                ...
                <input type="radio" name="f-type_post" id="f-type_post" value="1">
                    ...
                    <div class="row text-center pl-3 pr-3">
                        <label class="type-check col-6">
                            <input type="radio" name="f-type_post" onclick="myFunction(this.value)" 
                            <?php if($post->type_post == '1'){ echo 'checked'; } ?> value="1">
                        ...
                        <label class="type-check col-6">
                            <input type="radio" name="f-type_post" onclick="myFunction(this.value)" <?php if($post->type_post == '2'){ echo 'checked'; }?> value="2">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control pl-3 pr-3" style="background: #FF2547;color:#fff;height: 51px;box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.25);border-radius: 10px;" id="f-category" name="f-category">
                            <option>เลือกประเภทของ</option>
                            @if(!empty($categories))
                                @foreach($categories as $category)
                                    <option <?php if($post->category == $category->name){ echo 'selected'; }?> value="{{ $category->name }}">{{ $category->name }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control sy-input" style="height: 51px;box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.15);border-radius: 10px;" value="{{$post->title}}" name="f-title" placeholder="หาคนรับฝากซื้อ...." type="text" id="f-title">
                    </div>

                    <div class="form-group">
                        <label for="body" class="control-label"><i class="fas fa-file-signature"></i> เพิ่มรายละเอียดสินค้า</label>
                        <textarea class="form-control" style="box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.15);border-radius: 10px;" placeholder="รุ่น ยี่ห้อ Size  ความต้องการเพิ่มเติม..." name="f-description" cols="50" rows="5" id="f-description">{{$post->description}}</textarea>
                    </div>
                    
                    <div class="form-group mb-5">
                        <label for="due" class="control-label"><i class="far fa-image"></i> เพิ่มรูปภาพสินค้า</label>
                        <br>
                        <small>**หากไม่ต้องการเปลี่ยนรูปให้ข้ามการอัพโหลดนี้</small>
                        <form method="POST" action="/post/edit" style="margin-bottom:100px;" id="form-store" enctype="multipart/form-data">
                            {{ csrf_field() }}
            ...
            <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="f-place" name="f-place" style="border-radius: 10px;height: 51px;background: #FA3654;color:#fff;box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.25);">
                            <option {{$post->place == 'ข้อมูลสถานที่นัดหมาย'  ? 'selected' : ''}} value="ข้อมูลสถานที่นัดหมาย">ข้อมูลสถานที่นัดหมาย</option>
                            <option {{$post->place == 'อาคารเรียนรวม 1'  ? 'selected' : ''}} value="อาคารเรียนรวม 1">อาคารเรียนรวม 1</option>
                            <option {{$post->place == 'อาคารเรียนรวม 2'  ? 'selected' : ''}} value="อาคารเรียนรวม 2">อาคารเรียนรวม 2</option>
                            <option {{$post->place == 'อาคารยานยนต์'  ? 'selected' : ''}} value="อาคารยานยนต์">อาคารยานยนต์</option>
                            <option {{$post->place == 'โรงยิม'  ? 'selected' : ''}} value="โรงยิม">โรงยิม</option>
                            <option {{$post->place == 'โรงอาหาร หอ2'  ? 'selected' : ''}} value="โรงอาหาร หอ2">โรงอาหาร หอ2</option>
                            <option {{$post->place == 'หอพัก 1'  ? 'selected' : ''}} value="หอพัก 1">หอพัก 1</option>
                            <option {{$post->place == 'หอพัก 2'  ? 'selected' : ''}} value="หอพัก 2">หอพัก 2</option>
                            <option {{$post->place == 'หอพัก 3'  ? 'selected' : ''}} value="หอพัก 3">หอพัก 3</option>
                            <option {{$post->place == 'หอพัก 4'  ? 'selected' : ''}} value="หอพัก 4">หอพัก 4</option>
                            <option {{$post->place == 'หอพัก 5'  ? 'selected' : ''}} value="หอพัก 5">หอพัก 5</option>
                            <option {{$post->place == 'หอพัก 6'  ? 'selected' : ''}} value="หอพัก 6">หอพัก 6</option>
                            <option {{$post->place == 'Su Cafe'  ? 'selected' : ''}} value="Su Cafe">Su Cafe</option>
                            <option {{$post->place == '7-Eleven'  ? 'selected' : ''}} value="7-Eleven">7-Eleven</option>
                            <option {{$post->place == 'อาคารบริหาร'  ? 'selected' : ''}} value="อาคารบริหาร">อาคารบริหาร</option>
                            <option {{$post->place == 'สนามฟุตบอล'  ? 'selected' : ''}} value="สนามฟุตบอล">สนามฟุตบอล</option>
                            <option {{$post->place == 'สนามบาสเก็ตบอล'  ? 'selected' : ''}} value="สนามบาสเก็ตบอล">สนามบาสเก็ตบอล</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="body" class="control-label"><i class="fas fa-file-signature"></i> รายละเอียดสถานที่เพิ่มเติม</label>
                        <textarea class="form-control" style="box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.15);border-radius: 10px;" placeholder="รายละเอียดสถานที่เพิ่มเติม เช่น หน้าประตูทางเข้าหอ" name="f-description-pickup" cols="50" rows="5" id="f-description-pickup">{{$post->detail_place}}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="text" class="control-label"><i class="fas fa-hand-holding-usd"></i> ราคาสินค้า</label>
                        <input class="form-control" style="height: 51px;box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.15);border-radius: 10px;" value="{{$post->price}}" name="f-price" type="number" id="f-price">
                    </div> 
                    <div id="fristtype">
                        <div class="form-group">
                            <label for="text" class="control-label"><i class="fas fa-hand-holding-usd"></i> เสนอราคาหิ้ว</label>
                            <input class="form-control" style="height: 51px;box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.15);border-radius: 10px;" value="{{$post->fee}}" name="f-fee" type="number" id="f-fee">
                        </div>
                    </div>
                    <div id="secondtype"> 
                        <div class="form-group">
                            <label for="text" class="control-label"><i class="fas fa-file-signature"></i> จำนวนที่ต้องการ</label>
                            <input class="form-control" style="height: 51px;box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.15);border-radius: 10px;" value="{{$post->amount}}" name="f-amount" type="number" id="f-amount">
                        </div>
                        <div class="form-group">
                            <label for="body" class="control-label"><i class="fas fa-file-signature"></i> รายละเอียดสถานที่ซ้อสินค้า</label>
                            <textarea class="form-control" style="box-shadow: 0px 1px 10px rgba(0, 0, 0, 0.15);border-radius: 10px;" placeholder="อาคาร..." name="f-description-place" cols="50" rows="5" id="f-description-place">{{$post->buy_at}}</textarea>
                        </div>
                    </div>   
                    <div class="form-group mb-5">
                        <label for="text" class="control-label"><i class="far fa-clock"></i> เวลารับสินค้า : <span class="text-pink">{{ date("d/m/Y h:m:s", strtotime($post->pickup)) }}</span></label>
                        <br>
                        <small>**หากไม่ต้องการเปลี่ยนเวลาให้ข้ามการอัพโหลดนี้</small>
                        <input class="form-control" type="datetime-local" id="f-date-time"
                                ...
                                name="f-date-time" value="{{$post->pickup}}"
                                min="2021-01-01T00:00" max="2021-12-31T00:00">
                    </div>
            </div>
        </div>
    </div>
    <div id="part-three" style="margin-top:-30px!important;" class="position-relative">
        ...
        <div style="position: absolute; top: 3%; right: 7%">
            <button onclick="create()" class="btn btn-memove mt-3 mb-5 btn-block">แก้ไขโพส</button>
        </div>
        
        <img id="blah" style="width: 100%;" src="{{ url('') }}/uploads/photos/{{ $post->image}}" alt="" />
                    ...
                    <div class="row p-5 mb-5">
                        ...
                        <div class="col-12 mt-4">
                            ...
                            <span class="ml-3 float-left" style="line-height: 1.2">
                                ...
                                <br>
                                <label class="text-pink" style="font-weight: 300" id="text-time">{{ date("d/m/Y h:m:s", strtotime($post->pickup)) }}</label>
                            </span>
                        </div>
                        <div class="col-12 mt-4">
...
<script type="text/javascript">
    ...
    function nextTwo() {
        ...
        if(document.getElementById('f-date-time').value != null){
